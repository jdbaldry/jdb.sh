...
#+begin_src emacs-lisp emacs-lisp :results none
    ...
    (defun jdb/html--build-body (contents info &rest other)
      ...
      (setq node-content (dom-node "div" '((id . "content") (class . "content"))))
      (when (plist-get other :before)
        (dolist (node (plist-get other :before)) (dom-append-child node-content node)))
      (dolist (node (dom-children (dom-by-tag (jdb/html--parse-string contents) 'body))) (dom-append-child node-content node))
      (when (plist-get other :after)
        (dolist (node (plist-get other :after)) (dom-append-child node-content node)))
      (dom-node "body" () node-content))
      ...
      (when (and (plist-get info :html-htmlized-css-url)
        ...
        (dom-append-child node-head
                          (dom-node "link" `((rel . "stylesheet")
                                             ...
                                             (type . "text/css")))))
      (when (plist-get other :before)
        (dolist (node (plist-get other :before)) (dom-add-child-before node-head node)))
      (when (plist-get other :after)
...
#+begin_src css
  .layout {
    display: grid;
    grid-template-columns: 1fr /* nav */ 3fr /* content */;
  }
#+end_src
